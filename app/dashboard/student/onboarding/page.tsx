...
export default function StudentOnboardingPage() {
  ...
  const loadStudentData = async (uid: string) => {
    ...
    try {
      ...
      if (student) {
        ...
        // payment_method tidak disimpan di students, jadi tidak usah di-set
      } else {
  ...
  const finalizeOnboarding = async () => {
    ...
    const { data: updatedData, error: completeErr } = await supabase
      .from('students')
      .update({ onboarding_complete: true })
      .eq('user_id', userId)
      .select() // ambil data yang di-update untuk memastikan

    if (completeErr) {
      console.error('[ONBOARDING] ❌ Error update:', completeErr)
      throw new Error(`Gagal update status: ${completeErr.message}`)
    }
    console.log('[ONBOARDING] ✅ onboarding_complete berhasil diupdate:', updatedData)
    ...
    try {
      ...
      if (session) {
        ...
        if (!res.ok) {
          ...
          console.warn('[ONBOARDING] Payment API response not OK:', res.status, errText)
        } else {
  ...
  const handleNext = async () => {
    ...
    try {
      if (step === 1) {
        ...
        // Tidak perlu saveStep4Data karena payment API sudah dipanggil di finalize
        // Langsung finalisasi
        await finalizeOnboarding()
        console.log('[Onboarding] Finalisasi selesai, redirect ke dashboard...')
