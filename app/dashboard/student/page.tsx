...
export default function StudentDashboard() {
  ...
  // ============================================================
  // FUNGSI FETCH – PAKAI API ROUTE (bypass RLS)
  // ============================================================
  ...
  // ============================================================
  // FUNGSI UTAMA – dengan FALLBACK jika useAuth gagal
  // ============================================================
  const loadDashboardData = async () => {
    try {
      let userId: string | null = null

      // 1. Coba pakai useAuth dulu
      if (authUser?.id) {
        userId = authUser.id
        console.log('[StudentDashboard] ✅ Auth user dari useAuth:', userId)
      } else {
        // 2. Fallback: query langsung dari Supabase
        console.log('[StudentDashboard] ⚠️ useAuth tidak memberikan user, fallback ke Supabase')
        ...
        if (error) {
          ...
          if (session?.user) {
            ...
            console.log('[StudentDashboard] ✅ User dari session:', userId)
          } else {
            throw new Error('Tidak dapat mengambil user dari auth')
          }
        } else if (user) {
          userId = user.id
          console.log('[StudentDashboard] ✅ User dari supabase.auth.getUser():', userId)
        }
      }
      ...
      // 3. Ambil data student dari API (bypass RLS)
  ...
  // ============================================================
  // useEffect – dengan TIMEOUT FALLBACK 3 DETIK
  // ============================================================
  useEffect(() => {
    isMounted.current = true
    fetchDone.current = false

    // ⏱️ Timeout fallback agar dashboard tetap bisa dibuka
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8">
      ...
      <div className="p-3 bg-muted/30 border border-border rounded-md text-xs font-mono overflow-auto max-h-32">
        ...
        <p>User ID: <span className="text-blue-400">{debugUserId || '(tidak diketahui)'}</span></p>
        <p>Student Name: <span className="text-green-400">{studentName}</span></p>
        <p>Grade: {gradeLevel || '(kosong)'}</p>
        <p>Subjects: {subjects.length > 0 ? subjects.join(', ') : '(kosong)'}</p>
        <p>Onboarding Complete: {onboardingComplete ? '✅' : '❌'}</p>
      </div>
      ...
      {!onboardingComplete && studentName === 'Siswa' && (
