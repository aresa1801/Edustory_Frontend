...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        console.log('🔄 [Dashboard] Mulai fetch data...')
        ...
        if (userError) {
          ...
          if (session?.user) {
            console.log('👤 [Dashboard] User dari session:', session.user.id)
            await processUser(session.user)
          } else {
            throw new Error('Tidak dapat mengambil user')
          }
        } else if (user) {
          console.log('👤 [Dashboard] User dari getUser:', user.id)
    ...
    const processUser = async (user: any) => {
      try {
        ...
        console.log('🔍 [Dashboard] Mencari student dengan user_id:', user.id)

        // 2. Ambil data student dari API (bypass RLS)
        ...
        if (student) {
          ...
          if (student.id) {
            ...
            console.log('✅ [Dashboard] Matches & offers loaded')
          }
        } else {
        ...
        console.error('❌ [Dashboard] Error processUser:', err)
        setError(err.message || 'Gagal memuat data student')
      } finally {
        console.log('🏁 [Dashboard] Selesai, loading=false')
        setLoading(false)
      }
    }
