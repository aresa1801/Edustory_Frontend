...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        console.log('🔄 [DEBUG] Fetching data langsung dari Supabase...')
        ...
        // 2. 🔥 AMBIL DATA STUDENT LANGSUNG DARI SUPABASE (tanpa API)
        ...
        // 3. Update profile dengan data dari student (jika ada)
        if (studentData) {
          ...
          setProfile({
            ...
            email: '', // email tidak ada di students
  ...
  return (
    <div>
      ...
      <div className="mb-4 p-3 bg-muted/30 border border-border rounded-md text-xs font-mono overflow-auto max-h-40">
        <p className="font-bold text-foreground">📊 DEBUG: Data dari Supabase (langsung)</p>
        <pre>{JSON.stringify({
          id: profile.id,
          name: profile.name,
