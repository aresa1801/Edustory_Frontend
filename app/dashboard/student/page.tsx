...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    ...
    timeoutId.current = setTimeout(() => {
      if (isMounted.current && loading) {
        console.warn('⏱️ Timeout 3 detik, force loading=false')
        setLoading(false)
    ...
    const fetchData = async () => {
      try {
        ...
        // 2. 🔥 QUERY STUDENTS dengan user_id dari auth
        ...
        if (studentError) {
          console.error('❌ [DEBUG] Error query students dengan user_id:', studentError)
        }

        console.log('📦 [DEBUG] studentData dari query (dengan user_id):', studentData)
        setStudentFound(studentData)

        // 3. 🔥 AMBIL SEMUA STUDENTS (tanpa filter) untuk debug
        const { data: allStudentsData, error: allError } = await supabase
          .from('students')
          .select('id, user_id, name, grade_level')
          .limit(10) // batasi 10

        if (allError) {
          console.error('❌ [DEBUG] Error query all students:', allError)
        } else {
          console.log('📋 [DEBUG] Semua students (10 pertama):', allStudentsData)
          setAllStudents(allStudentsData || [])
        }

        // 4. Jika studentData ditemukan, update profile
        if (studentData) {
          ...
          setDebugInfo('✅ Student ditemukan dengan user_id yang cocok')
        } else {
          console.warn('⚠️ [DEBUG] Data student TIDAK DITEMUKAN dengan user_id:', user.id)
          setDebugInfo(`❌ Tidak ada student dengan user_id = ${user.id}`)
          
          // Cek apakah ada student dengan user_id yang berbeda
          if (allStudentsData && allStudentsData.length > 0) {
            const found = allStudentsData.find(s => s.user_id === user.id)
            if (found) {
              setDebugInfo(`✅ Sebenarnya ada student dengan user_id tersebut, tapi query maybeSingle gagal? Coba cek data: ${JSON.stringify(found)}`)
            } else {
              setDebugInfo(`❌ Tidak ada student dengan user_id = ${user.id}. Daftar student yang ada: ${allStudentsData.map(s => `${s.id} (user_id: ${s.user_id})`).join(', ')}`)
            }
          }
        }

        // 5. Ambil matches & offers (jika ada student id)
        if (studentData?.id) {
  ...
  return (
    <div>
      ...
      {/* 🔥 DEBUG PANEL – TAMPILKAN SEMUA INFORMASI */}
      <div className="mb-4 p-3 bg-muted/30 border border-border rounded-md text-xs font-mono overflow-auto max-h-80">
        <p className="font-bold text-foreground">📊 DEBUG: Data dari Supabase</p>
        <p><span className="text-blue-400">Auth User ID:</span> {authUserId || '(belum dapat)'}</p>
        <p><span className="text-yellow-400">Status:</span> {debugInfo || 'Memuat...'}</p>
        {studentFound ? (
          <pre>{JSON.stringify(studentFound, null, 2)}</pre>
        ) : (
          <p className="text-red-400">❌ Student dengan user_id tersebut tidak ditemukan</p>
        )}
        <hr className="my-2 border-border" />
        <p className="font-bold">📋 Daftar students (10 pertama):</p>
      ...
      {!studentFound && (
        <Alert className="mb-4 bg-red-500/10 border-red-500/30">
          <AlertDescription className="text-red-700 dark:text-red-300">
            ⛔ Data siswa untuk user ini tidak ditemukan di database. 
            Silakan <Link href="/dashboard/student/onboarding" className="font-bold underline">isi onboarding</Link> terlebih dahulu.
            <br />
            <span className="text-xs">Auth User ID: {authUserId || '(tidak diketahui)'}</span>
            <br />
            <span className="text-xs">Lihat daftar students di debug panel di atas untuk membandingkan.</span>
          </AlertDescription>
        </Alert>
      )}

      {!onboardingComplete && studentFound && (
            ...
            <Card className="border shadow-sm hover:shadow-md transition-shadow h-full relative overflow-hidden">
              ...
              <CardContent className="p-5 pt-2">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-2xl font-bold text-foreground">{profile?.name || 'Nama belum diisi'}</h3>
                ...
                <div className="space-y-1.5">
                  <p className="text-sm text-muted-foreground flex items-start gap-2">
                    ...
                    <span>
                      {profile?.subjects?.length > 0 ? profile.subjects.join(', ') : 'Belum ada mata pelajaran'}
                    </span>
                  </p>
                  <p className="text-sm text-muted-foreground flex items-start gap-2">
