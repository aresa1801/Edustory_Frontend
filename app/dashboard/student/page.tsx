...
export default function StudentDashboard() {
  ...
  // State namecard – terpisah (seperti tutor)
  ...
  const fetchDashboardData = async (userId: string) => {
    ...
    if (fetchDone.current) {
      console.log('[StudentDashboard] ⏭️ Fetch already done, skip.')
      return
    }
    ...
    try {
      ...
      if (!student) {
        ...
        // 🔥 Set loading false di sini juga
        if (isMounted.current) setLoading(false)
        return
      }

      // 🔥 Isi state terpisah
      console.log('[StudentDashboard] ✅ Setting student data:', student.name)
      ...
      if (student.id) {
        console.log('[StudentDashboard] 🔍 Fetching matches for student_id:', student.id)
      ...
      setError(null)
      console.log('[StudentDashboard] 🏁 Fetch completed successfully.')
  ...
  useEffect(() => {
    ...
    // ⏱️ Timeout fallback 4 detik (untuk jaga-jaga)
    timeoutId.current = setTimeout(() => {
      if (isMounted.current && loading) {
        console.warn('[StudentDashboard] ⏱️ Timeout 4 detik, force loading=false')
        setLoading(false)
    ...
    console.log('[StudentDashboard] ✅ authUser ready, user_id:', authUser.id)
  ...
  }, [authLoading, authUser]) // dependency: authLoading dan authUser
