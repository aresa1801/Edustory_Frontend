...
export default function StudentDashboard() {
  // ============================================================
  // 🔥 STATE NAMECARD – TERPISAH (LANGSUNG DARI FETCH)
  // ============================================================
  ...
  // ============================================================
  // FETCH DATA – LANGSUNG, TANPA useAuth, TANPA PROFILE
  // ============================================================
  useEffect(() => {
    let isMounted = true
    ...
    const loadStudentData = async (user: any) => {
      try {
        console.log('[StudentDashboard] 👤 User:', user.id)
        ...
        if (student) {
          console.log('[StudentDashboard] ✅ Student ditemukan:', student.name)
          // 🔥 SET STATE TERPISAH – LANGSUNG DARI DATA
          ...
          console.warn('[StudentDashboard] ⚠️ Student tidak ditemukan')
        ...
        console.error('[StudentDashboard] ❌ Error loadStudentData:', err)
        if (isMounted) {
          setError(err.message || 'Gagal memuat data student')
        }
      } finally {
        if (isMounted) {
          console.log('[StudentDashboard] 🏁 Selesai, setLoading(false)')
          setLoading(false)
          if (timeoutId) {
    ...
    // ⏱️ TIMEOUT FALLBACK 5 DETIK
    timeoutId = setTimeout(() => {
      if (isMounted && loading) {
        console.warn('[StudentDashboard] ⏱️ Timeout 5 detik, force loading=false')
        setLoading(false)
