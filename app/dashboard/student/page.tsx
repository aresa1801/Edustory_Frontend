...
export default function StudentDashboard() {
  ...
  // ============================================================
  // FETCH DATA – PALING SEDERHANA, PASTI SET LOADING FALSE
  // ============================================================
  useEffect(() => {
    let isMounted = true
    ...
    const fetchData = async () => {
      try {
        ...
        if (userError || !user) {
          ...
          await processUser(session.user, isMounted, supabase)
        } else {
          await processUser(user, isMounted, supabase)
        }
      } catch (err: any) {
        console.error('❌ [Dashboard] Error:', err)
        if (isMounted) {
          setError(err.message || 'Gagal memuat data')
          setLoading(false) // 🔥 PASTIKAN LOADING FALSE DI CATCH
        }
      }
    }

    const processUser = async (user: any, mounted: boolean, supabase: any) => {
      try {
        ...
        if (studentData) {
          console.log('✅ [Dashboard] Student ditemukan:', studentData.name)
          setProfile({
            id: studentData.id,
          ...
          console.warn('⚠️ [Dashboard] Student tidak ditemukan')
        }

        if (mounted) {
          setError(null)
          setLoading(false) // 🔥 PASTIKAN LOADING FALSE DI TRY
        }
      } catch (err: any) {
        console.error('❌ [Dashboard] processUser error:', err)
        if (mounted) {
    ...
    <div className="max-w-6xl mx-auto space-y-8">
      ...
      <div>
        ...
        <p className="text-muted-foreground">
          Selamat datang, {profile?.name || 'Siswa'}! Kelola pembelajaran dan cari tutor terbaik.
        </p>
      </div>
            ...
            <Card className="p-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-foreground">{profile?.subjects?.length ?? 0}</p>
                </div>
              </div>
            </Card>
          </div>
