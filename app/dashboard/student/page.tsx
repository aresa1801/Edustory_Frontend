...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        console.log('🔄 [Dashboard] Mulai fetch...')

        // 1. Ambil user
        let userId = authUser?.id
        if (!userId) {
          // Fallback: coba dari Supabase langsung
        ...
        if (isMounted) {
          ...
          // 🔥 PASTIKAN LOADING FALSE
          setLoading(false)
          console.log('🏁 [Dashboard] Error, loading=false')
        }
      }
    }

    // Tunggu authLoading selesai atau maksimal 3 detik
    const waitAndFetch = async () => {
      let attempts = 0
      while (authLoading && attempts < 15) {
  ...
  }, [authLoading, authUser]) // 🔥 DEPENDENSI: authLoading dan authUser
