...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (userError || !user) {
          ...
          await loadStudentData(session.user, isMounted, supabase)
        } else {
          await loadStudentData(user, isMounted, supabase)
        }
      } catch (err: any) {
    ...
    const loadStudentData = async (user: any, mounted: boolean, supabase: any) => {
      try {
        // 2. Query students langsung
        ...
        if (student) {
          ...
          console.log('✅ [Fetch] Student ditemukan:', student.name)
          ...
          console.warn('⚠️ [Fetch] Student tidak ditemukan')
        }

        if (mounted) {
          setError(null)
          setLoading(false)
          console.log('✅ [Fetch] Selesai, loading=false')
        }
      } catch (err: any) {
        console.error('❌ [Fetch] loadStudentData error:', err)
        if (mounted) {
  ...
  // RENDER LOGIC
  // ============================================================
  console.log('🔄 [Render] studentData:', studentData)

  // Helper
  const costPerSession = studentData?.budget_per_month && studentData?.sessions_per_month
    ? Math.round(studentData.budget_per_month / studentData.sessions_per_month)
    : 0
  ...
  // ============================================================
  const NameCard = () => {
    const name = studentData?.name || 'Siswa'
    const initial = name.charAt(0).toUpperCase()
    const avatar = studentData?.avatar_url || null
      ...
      <Card className="border shadow-sm hover:shadow-md transition-shadow h-full relative overflow-hidden">
        ...
        <CardContent className="p-5 pt-2">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground flex items-center gap-1">
                ...
                <span className="truncate">{studentData?.email || 'Email tidak tersedia'}</span>
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center text-sm text-muted-foreground mb-2">
            ...
            <span>{studentData?.grade_level || 'Kelas belum ditentukan'}</span>
          </div>
          <div className="space-y-1.5">
            <p className="text-sm text-muted-foreground flex items-start gap-2">
              <BookMarked className="w-4 h-4 mt-0.5 text-purple-400 flex-shrink-0" />
              <span>
                {studentData?.subjects?.length > 0 ? studentData.subjects.join(', ') : 'Belum ada mata pelajaran'}
              </span>
            </p>
            <p className="text-sm text-muted-foreground flex items-start gap-2">
              <MapPin className="w-4 h-4 mt-0.5 text-blue-400 flex-shrink-0" />
              <span>{studentData?.school_address || studentData?.address || 'Alamat belum diisi'}</span>
            </p>
            <p className="text-sm text-muted-foreground flex items-start gap-2">
              <Clock className="w-4 h-4 mt-0.5 text-orange-400 flex-shrink-0" />
              <span>{studentData?.preferred_schedule || 'Jadwal belum ditentukan'}</span>
            </p>
          </div>
  ...
  const onboardingComplete = studentData?.onboarding_complete || false
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8">
      ...
      <div className="p-3 bg-muted/30 border border-border rounded-md text-xs font-mono overflow-auto max-h-40">
        <p className="font-bold text-foreground">📊 DEBUG (Raw studentData)</p>
      ...
      <div>
        ...
        <p className="text-muted-foreground">
          Selamat datang, {studentData?.name || 'Siswa'}! Kelola pembelajaran dan cari tutor terbaik.
        </p>
      </div>
            ...
            <Card className="p-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-foreground">{studentData?.subjects?.length ?? 0}</p>
                </div>
              </div>
            </Card>
          </div>
