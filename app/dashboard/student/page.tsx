...
export default function StudentDashboard() {
  ...
  const fetchDashboardData = async (userId: string) => {
    if (fetchDone.current) {
      console.log('[StudentDashboard] ⏭️ fetchDone sudah true, skip')
      return
    }
    ...
    try {
      ...
      if (!student) {
        console.log('[StudentDashboard] ⚠️ Student tidak ditemukan')
        setStudentName(authUser?.user_metadata?.full_name || authUser?.email || 'Siswa')
        setOnboardingComplete(false)
        // jangan return dulu, biar lanjut ke finally
      } else {
        // Isi state terpisah
        console.log('[StudentDashboard] ✅ Mengisi state dengan data student')
        ...
        if (student.id) {
          console.log('[StudentDashboard] 🔍 Mengambil matches untuk student_id:', student.id)
          const { data: md, error: mdError } = await supabase
            ...
            .limit(5)
          if (mdError) console.warn('[StudentDashboard] ⚠️ Matches error:', mdError)
          ...
          const { data: offers, error: offersError } = await supabase
            ...
            .eq('status', 'pending')
          if (offersError) console.warn('[StudentDashboard] ⚠️ Offers error:', offersError)
      ...
      setError(null)
      console.log('[StudentDashboard] 🏁 Fetch selesai, akan set loading=false')

    } catch (err) {
      console.error('[StudentDashboard] ❌ Error:', err)
      setError(err instanceof Error ? err.message : 'Gagal memuat data dashboard')
    } finally {
      // 🔥 PASTIKAN LOADING FALSE DI SINI (tanpa syarat)
      console.log('[StudentDashboard] 🔥 finally: setLoading(false)')
      if (isMounted.current) {
        setLoading(false)
      } else {
        console.log('[StudentDashboard] ⚠️ Komponen sudah unmount, skip setLoading')
      }
    }
  }

  // useEffect – tanpa timeout
  useEffect(() => {
    ...
    if (authLoading) {
      console.log('[StudentDashboard] ⏳ authLoading true, menunggu...')
      return
    }
    ...
    console.log('[StudentDashboard] ✅ authUser ada, user_id:', authUser.id)
